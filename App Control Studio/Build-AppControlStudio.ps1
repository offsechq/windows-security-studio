...
.EXAMPLE
    .\Build-AppControlStudio.ps1 -X64Only
    Quick x64-only build for development.

.EXAMPLE
    .\Build-AppControlStudio.ps1 -X64Only -Install
...
if (-not $X64Only) {
    ...
    & $MSBuildPath 'eXclude\ComManager\ComManager.slnx' /p:Configuration=Release /p:Platform=arm64 /target:"clean;Rebuild" /v:minimal
...
if (-not $X64Only) {
    Write-Host "  Building Shell (ARM64)..." -ForegroundColor Yellow
    & $MSBuildPath 'eXclude\Shell\Shell.slnx' /p:Configuration=Release /p:Platform=arm64 /target:"clean;Rebuild" /v:minimal
    if ($LASTEXITCODE -ne 0) { throw "Failed building Shell for ARM64" }
}
#endregion
...
if (-not $X64Only) {
    ...
    dotnet msbuild 'App Control Studio.csproj' /t:Publish /p:Configuration=Release /p:RuntimeIdentifier=win-arm64 /p:AppxPackageDir="MSIXOutputARM64\" /p:GenerateAppxPackageOnBuild=true /p:Platform=ARM64 -v:minimal $mspdbArg
