...
.PARAMETER X64Only
    Build only for x64 architecture (faster builds for development).
...
.EXAMPLE
    ...
    Full build for both x64 and ARM64.

.EXAMPLE
    .\Build-SystemSecurityStudio.ps1 -X64Only
    Quick x64-only build for development.

.EXAMPLE
    .\Build-SystemSecurityStudio.ps1 -X64Only -Install
...
if ($vcVersions.Count -gt 0) {
    $arm64ToolsPath = [System.IO.Path]::Combine($VCToolsPath, $vcVersions[0].Name, 'bin', 'Hostx64', 'arm64', 'cl.exe')
    $hasArm64Tools = Test-Path $arm64ToolsPath
}

if (-not $X64Only -and -not $hasArm64Tools) {
    Write-Host "  [WARNING] ARM64 C++ build tools not installed" -ForegroundColor Yellow
    Write-Host "            To install: VS Installer > Modify > Individual Components >" -ForegroundColor Yellow
    Write-Host "            'MSVC v143/v144 - VS C++ ARM64 build tools'" -ForegroundColor Yellow
    Write-Host ""
    $response = Read-Host "  Continue with x64-only build? (Y/n)"
    if ($response -eq 'n' -or $response -eq 'N') {
        Write-Host "  Build cancelled. Install ARM64 tools and retry." -ForegroundColor Red
        exit 1
    }
    $X64Only = $true
    Write-Host "  [INFO] Proceeding with x64-only build" -ForegroundColor Cyan
} elseif (-not $X64Only) {
    Write-Host "  [OK] ARM64 C++ tools: Found" -ForegroundColor Green
}
$mspdbcmfPath = $null
if ($vcVersions.Count -gt 0) {
...
if (-not $SkipClean) {
    ...
    @('MSIXOutputX64', 'MSIXOutputARM64', 'MSIXBundleOutput', 'bin', 'obj') | ForEach-Object {
...
if (-not $X64Only) {
    Write-Host "  Building ComManager (ARM64)..." -ForegroundColor Yellow
    & $MSBuildPath '..\App Control Studio\eXclude\ComManager\ComManager.slnx' /p:Configuration=Release /p:Platform=arm64 /target:"clean;Rebuild" /v:minimal
    if ($LASTEXITCODE -ne 0) { throw "Failed building ComManager for ARM64" }
}
#endregion
...
if (-not $X64Only) { rustup target add aarch64-pc-windows-msvc 2>$null }
...
if (-not $X64Only) {
    Write-Host "  Building Rust Interop (ARM64)..." -ForegroundColor Yellow
    cargo build_arm64
    if ($LASTEXITCODE -ne 0) { Pop-Location; throw "Failed building ARM64 Rust Interop project" }
}
...
if (-not $X64Only) {
    Write-Host "  Building DISMService (ARM64)..." -ForegroundColor Yellow
    ...
    if ($LASTEXITCODE -ne 0) { throw "Failed building DISMService ARM64" }
    ...
    if ($LASTEXITCODE -ne 0) { throw "Failed publishing DISMService ARM64" }
}
...
if (-not $X64Only) {
    Write-Host "  Building QuantumRelayHSS (ARM64)..." -ForegroundColor Yellow
    ...
    if ($LASTEXITCODE -ne 0) { throw "Failed building QuantumRelayHSS ARM64" }
    ...
    if ($LASTEXITCODE -ne 0) { throw "Failed publishing QuantumRelayHSS ARM64" }
}
#endregion
...
if (-not $X64Only) {
    ...
    Write-Host "  Building ARM64 MSIX..." -ForegroundColor Yellow
    dotnet clean 'System Security Studio.csproj' --configuration Release
    ...
    if ($LASTEXITCODE -ne 0) { throw "Failed building ARM64 project" }

    dotnet msbuild 'System Security Studio.csproj' /t:Publish /p:Configuration=Release /p:RuntimeIdentifier=win-arm64 /p:AppxPackageDir="MSIXOutputARM64\" /p:GenerateAppxPackageOnBuild=true /p:Platform=ARM64 -v:minimal $mspdbArg
    if ($LASTEXITCODE -ne 0) { throw "Failed packaging ARM64 project" }
}
#endregion
...
if (-not $X64Only) {
    $arm64Msix = Get-ChildItem -Path '.\MSIXOutputARM64' -Recurse -Filter '*.msix' | Select-Object -First 1
    & $SignToolPath sign /fd $PackageHashAlgo /sha1 $Cert.Thumbprint /td $PackageHashAlgo /tr "http://timestamp.digicert.com" $arm64Msix.FullName
    if ($LASTEXITCODE -ne 0) { throw "Failed signing ARM64 MSIX" }
    Copy-Item -Path $arm64Msix.FullName -Destination '.\MSIXBundleOutput\' -Force
}
