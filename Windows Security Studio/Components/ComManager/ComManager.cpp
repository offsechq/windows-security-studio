...
int wmain(int argc, wchar_t* argv[])
{
	...
	// limited to RuleGroup="WindowsSecurityStudio", outputting JSON for.
	if (EqualsOrdinalIgnoreCase(primary.c_str(), L"firewallprogramlist"))
	{
		...
		bool ok = FW_ListProgramFirewallRulesInWindowsSecurityStudioGroupJson();
		if (!ok)
		{
			const wchar_t* err = GetLastErrorMessage();
			if (err && *err)
		...
		// FW_ListProgramFirewallRulesInWindowsSecurityStudioGroupJson writes JSON to stdout directly.
		return 0;
	}
