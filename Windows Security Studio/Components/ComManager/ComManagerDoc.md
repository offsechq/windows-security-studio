...
| `FirewallProgramList` | List program-based firewall rules from the same policy store used by this program, limited to the `WindowsSecurityStudio` group, outputting JSON |
...
- `RuleGroup = "WindowsSecurityStudio"`
...
- `RuleGroup` equals `"WindowsSecurityStudio"`
...
List all WindowsSecurityStudio program rules:
```
ComManager.exe firewallprogramlist
```
