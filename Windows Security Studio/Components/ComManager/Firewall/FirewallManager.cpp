...
namespace Firewall {
	...
	/// Deduplication Logic: if in the "WindowsSecurityStudio" group there is a rule for
	...
	)
	{
		...
		const wchar_t* targetGroup = L"WindowsSecurityStudio";
	...
	// filtered down to RuleGroup="WindowsSecurityStudio".
	// Output is written to stdout as a JSON array.
	// Returns true on success, false on error (error set).
	extern "C" __declspec(dllexport) bool __stdcall FW_ListProgramFirewallRulesInWindowsSecurityStudioGroupJson()
	{
		ClearLastErrorMsg();
