# Windows Firewall | Windows Security Studio
...
The **Management** tab provides an interface for creating, viewing, and deleting Windows Firewall rules created by the Windows Security Studio app. This feature is designed to give you granular control over network traffic for specific applications and directories.
...
> - **Organization**: All rules are tagged with the `WindowsSecurityStudio` group, making them easy to identify and manage exclusively through this application without cluttering the default Windows Firewall list.
